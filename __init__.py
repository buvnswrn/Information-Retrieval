...
#      Extracting features from the documents
      for doc_id,_ in score_1000[query]:
          ...
          feature_tf = 0
          for term in queries[query].keys():
              feauture_tfidf+=  term_frequency_unnormalized.get(doc_id,{}).get(term,0)*idf.get(term,0)/len(corpus[doc_id]) #/len(queries[query].keys())
...
def retrieval_SVM_sentence(sentence_raw,sentence_tokenized,avg_sl,score_sentence_BM25):
  ...
  for query in tqdm(queries.keys()):
      ...
      for sentence_id in sentence_tokenized[query].keys():
          ...
          term_frequency_sentence = Counter(sentence)
          feauture_tfidf = 0
          feature_tf = 0
#         Extracting features from the sentences
          for term in queries[query].keys():
              #if len(sentence) == 0: print('error: ',term_frequency_sentence)
              feauture_tfidf+=  term_frequency_sentence.get(term,0)*idf.get(term,0)/len(sentence) #/len(queries[query].keys())
              feature_tf += term_frequency_sentence.get(term,0)/len(sentence) #/ len(queries[query].keys())
...
PARAM_B = 0.75
EPSILON = 0.25
# collect idf sum to calculate an average idf for epsilon value. Applies only to BM25
idf_sum = 0
...
precision_50 = precision(score_50) # This contains the query numer as key and precision@50 for the corresponding query as value
print('\nMean Avg Precision@50 of the baseline model is {}'.format(sum(list(precision_50.values()))/len(list(precision_50.values()))))
...
print('\nMean Avg Precision@50 of BM25 is {}'.format(sum(list(precision_50_bm25.values()))/len(list(precision_50_bm25.values()))))
...
# Document Length (Normalised by average document length), Term Frequency, TFIDF, BM25 
...
print('\nMean Avg Precision@50 of SVM is {}'.format(sum(list(precision_50_svm.values()))/len(list(precision_50_svm.values()))))
...
mrr, ranklist = MRR(score_sentence_bm25_50)
print('\nMean Reciprocal Rank of BM25 is {}'.format(mrr))
print('\nRank of sentence for each query using BM25 is:\n{}'.format(ranklist))
sentence_bm25_50  = {}
...
sentence_svm_50  = {}
mrr, ranklist = MRR(score_sentence_svm_50)
print('\nMean Reciprocal Rank of SVM is {}'.format(mrr))
print('\nRank of sentence for each query using SVM is:\n{}'.format(ranklist))
